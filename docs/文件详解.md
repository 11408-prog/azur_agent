...
**当前覆盖 4 个模块，共 76 个测试用例：**

| 测试文件 | 用例数 | 覆盖的源文件 | 重点覆盖内容 |
|------|------|------|------|
| `test_tool_executor.cpp` | 42 | `core/tool_executor.cpp` | `resolveSafePath()` 路径穿越防护（越界拒绝/白名单放行）、`isBlacklistedCommand()` 黑名单（含短选项簇拆解、大小写不敏感）、`write_file`/`apply_patch` 的成功与失败路径、**`previewDiff()` 与真正执行结果一致性**（`PreviewDiff_ApplyPatch_MatchesActualExecution_*`，对应「五、2」的历史 bug 修复） |
| `test_context_policy.cpp` | 11 | `core/contextpolicy.cpp` | system 消息永不丢弃、最近 N 轮保护、tool 消息分级丢弃、tool_calls 与其 tool 结果的完整性绑定、空输入/极端参数不崩溃 |
| `test_appsettings.cpp` | 11 | `data/appsettings.cpp` | 各配置项默认值、读写往返（round-trip），验证「全项目唯一 QSettings 入口」没有读写不对称的问题 |
| `test_git_executor.cpp` | 12 | `core/git_executor.cpp` | `status`/`diff`/`log`/`commit`/`branch` 在真实临时 git 仓库中的行为，包括空仓库、无变更、非法仓库路径等边界情况 |

**测试构建方式的两个细节：**
- `tests/CMakeLists.txt` 里的 `TEST_SOURCES` 直接把被测源文件（`../core/tool_executor.cpp` 等）编译进测试可执行文件，而不是链接主程序产物——这意味着测试和主程序是两份独立编译产物，**改被测函数签名后要同时确认测试目标能编译过**，CI/本地跑一遍 `azur_agent_tests` 是唯一保证。
- `git_executor` 的测试会在系统临时目录创建真实 git 仓库并执行真实 `git` 命令（不是 mock），依赖运行环境里有可用的 `git` 可执行文件。

**已知问题：** 仓库中还留有一个 `tests/contextpolicy.cpp`（注意路径，不在 `core/` 下），内容是 `core/contextpolicy.cpp` 的旧版拷贝，但并未被 `tests/CMakeLists.txt` 的 `TEST_SOURCES` 引用（`TEST_SOURCES` 里写的是 `../core/contextpolicy.cpp`），实际不参与编译。这是一个孤立的历史遗留文件，容易让人误以为测试用的是这份代码，建议直接删除以免误导。

**尚未覆盖的模块：** `agent_engine.cpp`（Agent 循环、`isBusy()`/`autoExecute_` 状态机）、`ai_client.cpp`（SSE 流式解析）、`promptloader.cpp`、`project_analyzer.cpp`，以及所有 UI 层组件都还没有单元测试，目前只能靠手工验证。`agent_engine` 和 `ai_client` 涉及网络/异步信号槽，补测试的成本比纯函数类（`tool_executor`/`contextpolicy`）高一些，是后续测试建设的主要缺口。
