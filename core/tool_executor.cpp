...
QJsonArray ToolExecutor::toolDefinitions()
{
    ...
    // ---- git_status ----
    {
        ...
        QJsonObject func;
        func["name"] = "git_status";
        func["description"] =
            "显示当前 Git 仓库的状态（相当于 git status）。返回暂存区变更、"
            "工作区未暂存变更、未跟踪文件。无需参数，调用即可获得完整状态概览。";
        func["parameters"] = params;
    ...
    // ---- git_diff ----
    {
        QJsonObject stagedProp;
        stagedProp["type"] = "boolean";
        stagedProp["description"] = "设为 true 查看暂存区的 diff（即已 git add 的内容），默认 false 查看工作区未暂存的变更";

        QJsonObject pathsProp;
        pathsProp["type"] = "array";
        pathsProp["description"] = "要查看 diff 的文件路径列表（相对于工作区根目录），留空表示查看所有文件";
        QJsonObject pathsItems;
        pathsItems["type"] = "string";
        pathsProp["items"] = pathsItems;
        ...
        func["name"] = "git_diff";
        func["description"] =
            "查看代码的具体变更内容（相当于 git diff）。返回按文件分组的详细 diff。"
            "可指定文件路径只看特定文件的变更，或设置 staged=true 查看暂存区的 diff。";
        func["parameters"] = params;
    ...
    // ---- git_log ----
    {
        QJsonObject maxCountProp;
        maxCountProp["type"] = "integer";
        maxCountProp["description"] = "要显示的最近提交数量，默认 10，最大 100";

        QJsonObject branchProp;
        branchProp["type"] = "string";
        branchProp["description"] = "要查看的分支名，默认为当前分支";

        QJsonObject props;
        props["max_count"] = maxCountProp;
        ...
        func["name"] = "git_log";
        func["description"] =
            "查看 Git 提交历史（相当于 git log --oneline）。返回按时间倒序排列的提交列表，"
            "每条包含提交哈希和提交信息。可指定查看数量或查看特定分支的历史。";
        func["parameters"] = params;
    ...
    // ---- git_commit ----
    {
        ...
        messageProp["type"] = "string";
        messageProp["description"] = "提交信息，用中文描述本次变更的内容和目标";
        ...
        params["type"] = "object";
        params["properties"] = props;
        params["required"] = QJsonArray{ "message" };

        QJsonObject func;
        func["name"] = "git_commit";
        func["description"] =
            "暂存所有变更并创建一个新的 Git 提交（相当于 git add -A && git commit -m）。"
            "会自动暂存所有未跟踪和已修改的文件。注意：此操作会写入提交历史，需要用户确认。"
            "在调用此工具前，建议先调用 git_diff 让用户了解将要提交的变更内容。";
        func["parameters"] = params;
    ...
    // ---- git_branch ----
    {
        ...
        QJsonObject func;
        func["name"] = "git_branch";
        func["description"] =
            "列出所有本地 Git 分支（相当于 git branch）。当前分支会以 * 标记。无需参数。";
        func["parameters"] = params;
...
bool ToolExecutor::isWriteTool(const QString &toolName)
{
    ...
    bool result = toolName == QLatin1String("write_file")
           ...
           || toolName == QLatin1String("git_commit");
                                   ...
                                   const QJsonObject &arguments, bool *ok, QString *displayLabel)
{
    *ok = false;
    ...
    if (toolName == QLatin1String("git_commit")) {
        const QString message = arguments.value("message").toString().trimmed();
        if (message.isEmpty()) {
            *ok = false;
            return QStringLiteral("错误：commit message 不能为空");
        }

        // 先看一下当前状态，方便用户了解要提交什么
        bool statusOk = false;
        QString statusLabel;
        const QString statusOutput = GitExecutor::status(workspaceRoot, arguments,
                                                          &statusOk, &statusLabel);

        *ok = true;
        *displayLabel = QStringLiteral("git commit 预览");

        QStringList preview;
        preview << QStringLiteral("即将执行 git 提交：");
        preview << QStringLiteral("");
        preview << QStringLiteral("提交信息: %1").arg(message);
        preview << QStringLiteral("");
        preview << QStringLiteral("变更概况：");
        if (statusOk && !statusOutput.contains(QStringLiteral("工作区干净"))) {
            // 提取变更行
            const QStringList lines = statusOutput.split('\n');
            for (const QString &line : lines) {
                if (line.startsWith(QStringLiteral("  ")) || line.startsWith(QStringLiteral("*")))
                    preview << line;
            }
        } else {
            preview << QStringLiteral("  （无变更或无法获取状态）");
        }
        preview << QStringLiteral("");
        preview << QStringLiteral("提示：提交会写入 Git 历史，确认后不可撤销（除非 git reset）。");
                              ...
                              QString *diffOutput)
{
    ...
    // ---- Git 工具路由 ----
    if (toolName == QLatin1String("git_status")) {
        return GitExecutor::status(workspaceRoot, arguments, ok, displayLabel);
    }
    if (toolName == QLatin1String("git_diff")) {
        return GitExecutor::diff(workspaceRoot, arguments, ok, displayLabel);
    }
    if (toolName == QLatin1String("git_log")) {
        return GitExecutor::log(workspaceRoot, arguments, ok, displayLabel);
    }
    if (toolName == QLatin1String("git_commit")) {
        return GitExecutor::commit(workspaceRoot, arguments, ok, displayLabel);
    }
    if (toolName == QLatin1String("git_branch")) {
        return GitExecutor::branch(workspaceRoot, arguments, ok, displayLabel);
    }
