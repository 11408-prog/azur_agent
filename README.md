# Azur Agent 项目说明

> 基于 Qt6 + ElaWidgetTools 构建的 AI 角色扮演聊天软件。提示词是使用碧蓝航线的企业的台词构建的。目前还在开发阶段。
>
> 工具调用支持 C++ 原生与 Python 两种后端（默认 Python，可自动回退原生），详见 [关键设计决策](docs/关键设计决策.md) 第 7 条。

<br><br>
## 聊天界面展示

![聊天界面展示](app/avatar/show.png)

<br><br>
## 相关文档链接
<br>[项目概览](docs/项目概览.md)<br>
<br>[文件详解](docs/文件详解.md)<br>
<br>[架构总览](docs/架构总览.md)<br>
<br>[构建与运行](docs/构建与运行.md)<br>
<br>[数据流](docs/数据流.md)<br>
<br>[类关系图](docs/类关系图.md)<br>
<br>[常见问题与对策](docs/常见问题与对策.md)<br>
<br>[关键设计决策](docs/关键设计决策.md)<br>




