    ...
    , deleteBtn_(nullptr)
{
    ...
    // Qt 对话框里如果没有任何按钮显式设置 setDefault(true)，会把"第一个创建的、
    // autoDefault 为真的按钮"当成回车默认触发对象——这里"删除"是最先创建的按钮，
    // 导致用户按 Enter 时实际触发的是删除，而不是符合直觉的"打开"。
    ...
    // 显式声明"打开"才是这个对话框回车键的默认目标，符合"双击/回车=打开"的直觉
